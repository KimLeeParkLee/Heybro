...
HEYBRO는 **남성 맞춤형 스킨케어 루틴 추천, AI 톡봇, 쿠폰 및 포인트, 커뮤니티** 기능을 제공하는 **남성 그루밍 루틴 서비스**입니다.

사용자는 **온보딩 설문과 AI 피부 측정**으로 맞춤 루틴을 추천 받고, 루틴 완료 시 받은 포인트로 쿠폰을 구매할 수 있습니다.

루틴 중 궁금한 점이 생긴다면 브로톡인 커뮤니티와 마이브로인 AI 톡봇을 통해 해결할 수 있습니다.
...
- 백엔드
    ...
    **N+1 쿼리 문제 해결**
    ...
    - 상황 : 특정 질문 게시글 상세 조회 API에서 게시글 작성자 정보와 이미지 목록을 가져올 때 **각 질문마다 별도의 쿼리가 추가적으로 발생**하는 ****N+1 쿼리 문제 발생
    - 원인 : 게시글과 연관된 작성자, 이미지 정보가 **지연 로딩**으로 설정되어 있어 게시글을 조회한 뒤 각 정보에 접근할 때마다 추가적인 쿼리 발생
    - 해결 : **LEFT JOIN FETCH**를 사용하여 단 한 번의 쿼리로 연관된 정보(게시글 작성자, 이미지 목록)를 모두 함께 가져오도록 조회
    - API의 99% 응답 시간을 **211ms에서 53ms로 단축**시킴
    
    **트랜잭션 커밋 지연 문제**
    
    - 상황 : 온보딩 결과 전송 API에서 루틴 로그 생성 메서드를 호출했으나, **오늘의 루틴 로그가 생성되지 않는 문제**가 발생
    - 원인 : **@Transactional**이 적용되어 있어 루틴 로그 생성 메서드 호출 시점에는 트랜잭션 내에 루틴 정보가 반영되지 않아 로그를 생성할 데이터를 찾지 못함
    - 해결 : 루틴 로그 생성 작업이 부모 트랜잭션과 독립적으로 커밋되도록 **@Transactional(propagation = Propagation.REQUIRES_NEW)**를 적용
