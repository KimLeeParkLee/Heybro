...
- @Transactional의 기본 전파 레벨인 **REQUIRED**로 적용되어 있어, 루틴 로그 생성 메서드 호출 시점에는 **영속성 컨텍스트의 1차 캐시**에만 저장 -> findAll()로 DB에서 데이터를 조회하지 못함
- 1차 시도로 루틴 로그 생성 작업이 부모 트랜잭션과 독립적으로 커밋되도록 @Transactional(propagation = Propagation.REQUIRES_NEW)를 적용했지만 **데이터 정합성**에 대한 문제가 있을 수 있었음
- 근본적인 원인인 불필요한 DB 조회 대신 루틴 로그 생성 메서드에 파라미터로 직접 전달하는 방식으로 수정
